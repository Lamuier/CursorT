...
- 修复任务对话详情只显示创建时的第一条用户消息：改为同时请求官方 Cloud Agents 对话接口，并合并助手回复与后续跟进；详情页气泡使用稳定唯一键，避免后续消息被丢掉
- 任务页增加「新建」按钮，可从本机填写说明与仓库后启动云端任务
- 「在网页打开 Cursor Agents」与「查看 PR」使用系统浏览器打开已校验的 https 链接，不依赖 Chrome Custom Tabs

...
> 版本判定：2.5.5 → 2.5.6。本版将「概览」模型用量圆环拆成自有 / 三方两个独立进度环，不新增业务能力、无不兼容变更，按 SemVer 定修订版。

- 「概览」英雄卡改为并排两个圆环：自有（官方 `autoPercentUsed`）与三方（官方 `apiPercentUsed`），各自按用量档位着色
- 计费周期进度仍在圆环下方的周期行展示，不再叠在用量环内侧

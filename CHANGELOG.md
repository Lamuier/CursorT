...
> 版本判定：2.7.0 → 2.8.0。本版新增状态小组件 4×1 / 4×2 / 2×2 规格，属向后兼容的新用户可见能力，按 SemVer 定次版本。

- 状态小组件新增 4×1 横条、4×2 中卡、2×2 方卡；原有 2×1 迷你条与 4×3 详情卡保留
- 新增概览圆环配色说明（`docs/overview-ring-colors.md`）
- `versionCode=26`，`versionName=2.8.0`

## 2.7.0

> 版本判定：2.6.2 → 2.7.0。本版新增分列圆环在自有池用尽时自动合并、小组件 15 分钟后台刷新，以及打开应用后同步小组件，属向后兼容的新用户可见能力，按 SemVer 定次版本。

- 概览选择分列圆环时，若自有池已用尽而三方仍有额度，本周期自动改为合并显示，下个周期再回到分列
- 桌面小组件增加 15 分钟后台周期刷新；打开应用并拉到最新用量/状态后会立刻同步小组件
- 下拉刷新只保留页面顶部指示，点顶栏刷新只转右上角按钮，两处不再同时出现刷新动画
- `versionCode=25`，`versionName=2.7.0`

## 2.6.2

> 版本判定：2.6.1 → 2.6.2。本版修复概览分列圆环图例与短弧呼吸，并调整周期日期展示，不新增业务能力、无不兼容变更，按 SemVer 定修订版。

- 修复概览分列圆环：补回内环对应的「周期 %」图例，与合并总用量一致
- 概览计费周期改为与圆环同轴居中：剩余时间作主信息，起止日期作说明，去掉列表式标题、徽章与分割线
- 概览周期起止改为「09/09 — 10/09」与「23:15 GMT+8」分层，日期不再和时间挤成一行
- 界面日期改用斜杠（`09/09`、`2026/12/01`）；存储与接口仍是 ISO 连字符
- 修复概览分列圆环：较短一侧永远叠在上面，呼吸时不再被较长一侧整段盖住后闪断
- `versionCode=24`，`versionName=2.6.2`

## 2.6.1

> 版本判定：2.6.0 → 2.6.1。本版修复概览分列圆环被较长一侧盖住，不新增业务能力、无不兼容变更，按 SemVer 定修订版。

- 修复概览分列圆环：自有与三方改为反向呼吸交替显示，较长一侧不再把较短一侧盖死
- `versionCode=23`，`versionName=2.6.1`

## 2.6.0

> 版本判定：2.5.7 → 2.6.0。本版新增概览圆环样式切换与时区搜索选择，属向后兼容的新用户可见能力，按 SemVer 定次版本。

- 设置新增「概览圆环」：默认仍分列自有 / 三方（同环叠画），可换回原来的总用量单环（中心为总用量与健康度，图例为用量 / 周期）
- 设置里时区只固定展示「跟随系统」，其余时区改用搜索框按城市 / 名称筛选选择
- `versionCode=22`，`versionName=2.6.0`

## 2.5.7

> 版本判定：2.5.6 → 2.5.7。本版修复「概览」计费周期起止被剩余徽章挤成省略号，不新增业务能力、无不兼容变更，按 SemVer 定修订版。

- 「概览」计费周期改成两行：标题与剩余徽章同行，起止日期单独占满宽；范围标签只在终点标一次时区（`09-09 — 10-09 15:41 GMT+8`），避免 `09-09 GMT+8 — 10-…` 显示不全
- 「账单」页周期卡片使用同一套范围文案
- `versionCode=21`，`versionName=2.5.7`

## 2.5.6

> 版本判定：2.5.5 → 2.5.6。本版将「概览」改为同一圆环两层：外环合并自有 / 三方用量（两段从起点并排叠画、互相覆盖），内环套餐周期，不新增业务能力、无不兼容变更，按 SemVer 定修订版。

- 「概览」英雄卡外环同时画自有（`autoPercentUsed`）与三方（`apiPercentUsed`）：从同一 12 点方向叠画，重合处由后画的覆盖；内环仍是套餐周期进度
- 图例分列自有 / 三方；周期起止与剩余天数在圆环下方的周期行展示
- `versionCode=20`，`versionName=2.5.6`

## 2.5.5

> 版本判定：2.5.4 → 2.5.5。本版调整状态小组件近期故障与指示灯展示，不新增业务能力、无不兼容变更，按 SemVer 定修订版。

- 状态详情小组件「近期故障」改为受影响模块名与发生时间，不再展示故障原因标题；进行中事件与计划维护仍显示官方标题
- 状态迷你条 / 详情卡右侧进度环改为彩色状态指示灯（正常 / 性能下降 / 故障 / 维护），用量小组件仍用进度环
- `versionCode=19`，`versionName=2.5.5`

## 2.5.4

> 版本判定：2.5.3 → 2.5.4。本版去掉「概览」套餐额度卡片，不新增业务能力、无不兼容变更，按 SemVer 定修订版。

- 「概览」去掉套餐额度 / 已用额度 / 剩余额度那一张卡片；额度构成仍在「用量」页
- `versionCode=18`，`versionName=2.5.4`

## 2.5.3

> 版本判定：2.5.2 → 2.5.3。本版对齐「概览」与「用量」的套餐额度数字，不新增业务能力、无不兼容变更，按 SemVer 定修订版。

- 「概览」额度卡改为套餐额度 / 已用额度 / 剩余额度，与「用量」额度构成使用同一套官方数字
- 「用量」额度构成按套餐额度收敛：包含用量 +（计入额度的）Bonus + 剩余 = 概览套餐额度；未计入额度的 Bonus 单独列出
- 周期 `limit` 缺失时两边都回退套餐 `includedAmount`，避免概览有额度、用量构成却为 0
- Token 自有池 / 三方池费用仍在概览单独一行，并标明与套餐额度不是同一组数字
- `versionCode=17`，`versionName=2.5.3`

## 2.5.2

> 版本判定：2.5.1 → 2.5.2。本版修复状态小组件点击后未打开状态页，不新增业务能力、无不兼容变更，按 SemVer 定修订版。

- 点击状态迷你条 / 状态详情小组件会打开应用并切到「状态」页签；用量小组件仍打开「概览」
- 应用已在后台时同样生效（`singleTask` / `onNewIntent`）
- `versionCode=16`，`versionName=2.5.2`

## 2.5.1

> 版本判定：2.5.0 → 2.5.1。本版调整已有用量档位阈值与提示条件，不新增业务能力、无不兼容变更，按 SemVer 定修订版。

- 「请关注用量」阈值由 70% 调整为 80%，与通知提醒对齐
- 用量百分比高于计费周期进度时同样提示请关注用量（90% / 100% 仍分别为即将用尽 / 已用尽）
...
> 版本判定：2.3.0 → 2.4.0。本版为「任务」页分组，并纳入 Grok Bot 发起的云端任务，属向后兼容的新用户可见能力，按 SemVer 定次版本。

- 「任务」页支持按仓库 / 状态 / 时间 / 来源分组，分组可折叠，选择保存在本机
- 任务列表与相对时间统一按最近活动 / 更新 / 创建时间降序
- 请求云端任务列表时带上 `include_sources`（含 `BACKGROUND_COMPOSER_SOURCE_GROK_BOT`），官网默认隐藏的 Grok Bot / API / SDK 来源会出现在列表中
- 任务卡片展示来源标签；Grok Bot 发起的任务单独高亮，页眉显示 Grok Bot 数量
- `versionCode=13`，`versionName=2.4.0`

## 2.3.0

> 版本判定：2.2.1 → 2.3.0。本版在「总览 / 用量」新增 Grok Bot 每周独立额度，属向后兼容的新用户可见能力，按 SemVer 定次版本。
...
> 版本判定：2.2.0 → 2.2.1。本版为总览页额度卡与 Token 卡片的展示优化，不新增业务能力、无不兼容变更，按 SemVer 定修订版。

- 「总览」页额度卡改为三列展示套餐额度、自有池消费与三方池费用，与下方 Token 卡片对齐；已消费按模型拆分——Composer / Grok / Auto 计入自有池，其余第三方模型计入三方池，Token 明细不可用时显示「—」
- 「输入 / 输出 / 缓存 Token」三张半宽卡片合并为一张全宽卡片，内部三列均分展示
- `versionCode=11`，`versionName=2.2.1`

## 2.2.0

> 版本判定：2.1.0 → 2.2.0。本版在「用量」页新增本计费周期按模型汇总的 Token 明细（输入 / 输出 / 缓存与费用），属向后兼容的新用户可见能力，按 SemVer 定次版本。

- 新增 Token 用量：调用 Cursor `DashboardService/GetAggregatedUsageEvents`，在「用量」页展示本周期总输入 / 输出 / 缓存 Token、合计费用，以及按模型排序的明细行
- Token 接口失败时不影响花费与百分比用量；旧缓存无该字段时显示「暂时不可用」提示，刷新成功后写入加密用量缓存
- `versionCode=10`，`versionName=2.2.0`

## 2.1.0

> 版本判定：2.0.0 → 2.1.0。本版新增「任务」页签（列表概览，点击后用 Chrome Custom Tabs 打开官方对话）以及可自定义页签顺序，属向后兼容的新用户可见能力，按 SemVer 定次版本。

- 新增「任务」页签：查看当前账号下的 Cursor 云端任务（后台智能体），展示任务名、执行状态（创建中 / 运行中 / 已完成 / 已出错 / 已过期）、仓库与分支、PR 状态（可跳转）、增删行数、改动文件数、所用模型与最近活跃时间
- 点击任务通过 Chrome Custom Tabs 打开官方 Agents 对话页，完整对话走浏览器登录态，避免应用内只能看到创建时的第一条提示
- 「在网页打开 Cursor Agents」与「查看 PR」同样经白名单校验后用 Custom Tabs 打开
- 主界面功能页签顺序可在设置中自定义（上移 / 下移，可恢复默认），当前停留的页签会跟着一起移动；顺序保存在本机
- 任务数据复用现有 Access Token 的会话认证获取列表（与账单接口同源），无需新增凭据；随前台静默刷新与手动刷新同步更新，本机加密缓存支持离线回显

...
- 「概览 / 用量 / 账单 / 任务 / 状态」五页签，带流畅切换动效；双环展示用量与计费周期进度。
- 「任务」页展示 Cursor 云端任务（后台智能体）：列表概览可点进查看完整对话（含助手回复与后续跟进），未过期任务可发送跟进；右上角「新建」可从本机启动云端任务。
...
- **最小收集**：仅缓存解析后的用量数字与任务摘要，不保存任何原始响应；任务对话只在查看时请求，不落盘。

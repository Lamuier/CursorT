...
- 总用量、计费周期、Credits、两个用量池（Cursor 模型 / 其他模型）、Grok Bot 每周独立额度与 On-demand 预算，集中呈现；套餐额度构成在「用量」页。
- 「用量」页另含本计费周期 Token 汇总：按模型展示输入 / 输出 / 缓存 Token 与费用；也可按自然月或历史计费周期回看 Token，并按费用估算两池占比。Grok Bot 周额度单独成节，不混入月度用量池。
- 「概览 / 用量 / 账单 / 任务 / 状态」五页签，带流畅切换动效；顺序可在设置中自定义；概览外环默认同环叠画自有 / 三方用量并反向呼吸交替显示，也可在设置中换回原来的总用量单环；内环展示套餐周期进度。
- 「任务」页展示 Cursor 云端任务（后台智能体）列表，可按仓库 / 状态 / 时间 / 来源分组；已合并分支会从列表移除。Grok Bot、API、SDK 等官网默认隐藏的来源也会列入。点击任务会用 Chrome Custom Tabs 打开官方 Agents 页面查看完整对话。
...
- 提供用量与状态两套 MD3 小组件。用量为 2×1 迷你条与 4×3 详情卡；状态为 2×1、2×2、4×1、4×2 与 4×3。Android 12+ 跟随系统配色。状态小组件无需 Access Token。
...
- 浅色 / 深色跟随系统，支持无障碍与大屏布局；主界面功能页签顺序、概览圆环样式、时间展示时区与界面语言可在设置中调整（时区默认跟随系统，其他时区可搜索）。默认简体中文；跟随系统时，英文系统显示英文，其余回退中文。
